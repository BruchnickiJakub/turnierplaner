...
function resolveTournamentId(resolved: { id?: unknown }): string | null {
...
export default async function TurnierBearbeitenPage({ params }: Props) {
  ...
  const id = resolveTournamentId(resolvedParams);
  if (!id) {
    notFound();
  }
  ...
  if (error) {
    console.error("[turniere/bearbeiten] Supabase:", error.message, id);
    notFound();
  }
  if (!data) {
    notFound();
      ...
      <div className="mt-8 w-full rounded-2xl border border-app-border/90 bg-app-card p-4 shadow-sm sm:p-8">
        <TournamentWizard
          ...
          initial={{
            ...
            tournamentStartAt: t.tournament_start_at,
            groupMatchDurationMinutes: t.group_match_duration_minutes,
            koMatchDurationMinutes: t.ko_match_duration_minutes,
          }}
        />
      </div>
    </div>
